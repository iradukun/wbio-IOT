...
CCodec8::CCodec8(const char* zDeviceId, SOCKET socket)
	: m_nFrameSize(0)
	, m_Socket(socket)
...
template<typename T>
inline void CCodec8::Push(T data)
{
...
template<>
inline void CCodec8::Push(std::string data)
{
...
const uint8_t* CCodec8::GetFrame() const
{
	return m_pFrame;
}

unsigned CCodec8::GetFrameSize() const
{
	return m_nFrameSize;
}

bool CCodec8::SendPosition(const double fLongitude, const double fLattitude, const double fAltitude, const int nPriority, const double fSpeed)
{
	Raz();
	Push<uint32_t>(0); // Preamble
	Push<uint32_t>(0); // Data Field Length
	Push<uint8_t>(0x08); // Codec ID
	...
	Push<IOcount_t>(5);	// N of Total IO
...
bool CCodec8::Request(int nResultSize)
{
...
uint16_t CCodec8::Crc16(const uint8_t* buffer, int len)
{
#if 1
	unsigned CRC = 0;
