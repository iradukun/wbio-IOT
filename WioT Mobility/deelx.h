...
#else
template<typename T>
static __inline T min(T a, T b)
{
	return (a < b) ? a : b;
}
template<typename T>
static __inline T max(T a, T b)
{
	return (a > b) ? a : b;
}
#endif
	...
	case RCHART('['):
		{
			...
			if( flags & IGNORECASE )
			{
				...
				for(i=0; i<oldcount; i++)
				{
					...
					{
						newmin = static_cast<int>(tolower(max(RCHART('A'), ranges[i * 2])));
						newmax = static_cast<int>(tolower(min(RCHART('Z'), ranges[i * 2 + 1])));
					...
					if( ranges[i*2] <= RCHART('z') && ranges[i*2+1] >= RCHART('a') )
					{
						newmin = static_cast<int>(toupper( max(RCHART('a'), ranges[i*2  ]) ));
						newmax = static_cast<int>(toupper( min(RCHART('z'), ranges[i*2+1]) ));
				...
				for(i=0; i<oldcount; i++)
				{
					if( isupper(chars[i]) && ! pRange->IsContainChar(static_cast<int>(tolower(chars[i]))) )
						chars.Push(static_cast<int>(tolower(chars[i])));

					if( islower(chars[i]) && ! pRange->IsContainChar(static_cast<int>(toupper(chars[i])) ))
						chars.Push(static_cast<int>(toupper(chars[i])));
				}
			}
